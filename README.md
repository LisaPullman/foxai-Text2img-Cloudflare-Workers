...
- 🔢 一次生成 1 / 2 / 4 张：并发池调度（同时最多 2 张在途、自动错峰）+ 限流自动退避重试，结果网格展示，单张失败可独立重试，支持一键下载全部
- 🖼 常用画面比例一键预设（1:1、4:3、16:9 等 7 档，随模型能力自动适配；像素模型点击即套用对应分辨率，也可拖动滑块自定义）
- 🔍 结果网格悬停可查看大图 / 单张下载，灯箱模式支持 ←/→ 切换、Esc 关闭
- ⌨️ 支持 Ctrl/⌘ + Enter 快捷生成、生成期间防重复提交、参数本地记忆
- 📦 页面零外部依赖（无 CDN 引用），加载快且无单点故障
- 🤗 支持设置访问密码，私有化部署友好
- ⚡ 响应速度快，全球边缘网络加速
- 🌓 支持深色和浅色模式（自动跟随系统 + 自动/亮/暗三态手动切换，无闪烁）
- 📱 移动端深度适配（触屏操作常显、防聚焦自动缩放、全面屏安全区留白）
...
3. **选择模型**：从下拉菜单中选择想要使用的文生图模型。页面每次打开默认选中 FLUX.1 [schnell]（免费额度内速度最快的推荐模型），切换后仅在本次会话内保持，其余参数仍按本地偏好记忆。模型下方会显示该模型支持的能力（尺寸范围、步数、引导系数、随机种子、画面比例等），标注"付费 Partner 模型"的选项需要账户启用付款方式。

4. **选择画面比例**：模型卡片下方提供常用比例预设按钮（1:1、4:3、16:9 等 7 档）。像素模型点击即套用对应分辨率（如 16:9 → 1344×768，Lucid Origin 为全高清档），右侧同步显示当前尺寸；FLUX.2 [dev] 直接选择输出比例。也可展开"高级选项"拖动滑块微调，与预设不一致时按自定义尺寸输出。

5. **生成图像**：在"生成数量"中选择一次生成 1 / 2 / 4 张（额度消耗相应倍增；支持随机种子的模型会自动以基准种子 + 序号派生不同种子，保证多张互不相同）。为避免触发 Cloudflare AI 并发限流，多张按并发池调度——同时最多 2 张在途并自动错峰；个别张被限流（429）时会指数退避自动重试（最多 2 次），仍失败可在格内手动重试。点击"生成图像"按钮或按 `Ctrl/⌘ + Enter`，右侧以网格展示各张进度，生成期间按钮自动禁用。

6. **管理结果**：悬停任一张可"查看大图"或"下载此图"；点击图片打开灯箱浏览（←/→ 切换、Esc 关闭）。顶部按钮可"下载图像 / 下载全部"或"复制参数"（文件扩展名自动匹配 PNG/JPEG，多张时文件名带序号）。个别张数失败时可在对应格内点"重试"，无需整批重来。

7. **切换主题**：通过页面右上角的"自动 / 亮 / 暗"三态切换选择主题模式；"自动"实时跟随系统明暗变化，选择会被记住。
...
1. **模型设置**：在 `src/worker.js` 的 `AVAILABLE_MODELS` 中可添加、删除或修改模型。每个模型除了基本信息外，还通过 `capabilities` 描述其能力——这是**唯一事实来源**，服务端据此校验钳制入参，前端（`/api/models` 下发后）据此显隐控件、调整滑块范围：
   ...
   ```js
   {
     ...
     capabilities: {
       ...
       size: false,                    // 或 { min, max, step, defaultWidth, defaultHeight, presets }
   ...
   `size.presets` 为像素模型的画面比例预设按钮（`[{ label, w, h }]` 数组）：前端在模型卡片下方渲染为一键按钮，点击即套用对应宽高。预置了 `SIZE_PRESETS_1024`（SD/FLUX 系通用档）与 `SIZE_PRESETS_FHD`（Lucid Origin 全高清档）两组，可按需增删——数值须在该模型 `min/max` 范围内并对齐 `step`，服务端仍会统一钳制校验。
...
- Workers AI 免费额度为每日 10,000 神经元（自动重置），日常使用建议私有化部署并设置访问密码，避免额度被他人消耗。一次生成多张时，额度与计费均按张数倍增。
